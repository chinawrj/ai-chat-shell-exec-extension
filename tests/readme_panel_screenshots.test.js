...
const previewPath = "docs/release-assets/v0.11.19/drawio-preview.png";
assert.ok(readme.includes(previewPath), "README must show the current Draw.io PNG/page controls.");
assert.ok(e2eSource.includes('"drawio-preview.png"'), "The preview image must be generated by real Chrome E2E.");
const previewImage = fs.readFileSync(path.join(root, previewPath));
assert.equal(previewImage.subarray(0, 8).toString("hex"), "89504e470d0a1a0a");
assert.ok(previewImage.readUInt32BE(16) >= 760 && previewImage.readUInt32BE(16) <= 1600);
assert.ok(previewImage.readUInt32BE(20) >= 500 && previewImage.readUInt32BE(20) <= 1300);
