...
- Frontend `activeCallId` covers backend execution only, never the potentially slow composer auto-send calibration loop. Shell/board responses enter a bounded per-tab persistent pending-delivery queue before composer work; retries are local and preserve exact ownership. Auto-send is a cancellable UI follow-up and must verify exact text, current composer association, and lifecycle before every side effect. A pending agent delivery defers, but does not mark or consume, a newly detected helper; submitted agent prompts are acked without reinsertion, identical agent-message response-loss retries are idempotent, and a page-agent profile change cancels the old delivery lifecycle.
