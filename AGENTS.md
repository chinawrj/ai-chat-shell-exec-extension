...
- Draw.io helpers use `ai-helper-drawio-start` or `ai-helper-drawio-start:<identity>`, the complete native `<mxfile>` XML body, and `ai-helper-drawio-end`. They are non-executable extension-local artifacts: never forward them to background/server/tmux, never write them into the composer, and never claim the AI can see the render. After scan quiet, select only the last complete XML-valid candidate. Render it with the pinned packaged viewer in the sandboxed `drawio/viewer.html` iframe, stage changed artifacts offscreen, and atomically swap only after fresh SVG readiness. Streaming, invalid XML, stale completion, and renderer failure must retain the previous SVG without flicker and expose bounded preview plus browser-console error diagnostics. Same XML artifacts are idempotent across DOM redraws; Close/Reopen never rescans or contacts a backend. Keep the pinned viewer's Apache-2.0 license, attribution, hash, and upstream asset/trademark terms in `extension/vendor/drawio/` plus `THIRD_PARTY_NOTICES.md`; the Chrome extension release ZIP must include `drawio/` and `vendor/`.
...
- The floating panel is compact and state-driven by default: normal idle shows only concise status and More; Server Check appears on an error, Force run appears when the page has a runnable helper and no shell helper is active, and Stop helper replaces Force run during active shell execution. Existing setup/recovery (including an always-available Server Check), page-binding, Agent/tmux-ai, full-status, and debug controls remain present under four labelled advanced groups and are never removed merely to reduce the default footprint. Draw.io is a contextual action: hidden until a preview artifact or error log exists, then retained for reopening after Close. The output-idle notice adds only contextual Continue waiting; Stop helper remains the single user-facing termination concept.
