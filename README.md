...
Discovered Skills are **uninstalled by default**. Open `View Skills` in the extension panel: each row shows `Install`, `Installing…`, `Installed`, `Retry`, or `No installer`. Installation is available only when a real, non-symlinked `install.sh` sits beside `SKILL.md`. The extension accepts only a trusted browser click and then shows a native confirmation naming the exact Skill. After confirmation it sends the Skill id, `SKILL.md` SHA, `install.sh` SHA, and catalog SHA to the shell server. The server snapshots the exact verified installer bytes, runs that private immutable snapshot from the Skill directory with a minimal non-secret environment, captures a bounded tail of stdout/stderr, and removes it. Installation has no fixed total-duration cutoff: the process group is terminated only after **600 consecutive seconds with no stdout or stderr**, and either stream resets that idle clock even after the diagnostic capture limit is reached. The extension keeps the long WebSocket alive without a shorter browser watchdog. Only an explicit exit code `0` with no termination signal or idle timeout, followed by fresh unchanged Skill/installer revalidation, records the Skill as installed. A nonzero exit, signal, or output-idle timeout leaves the Skill uninstalled and opens a one-use `chrome-extension://` result window with sanitized local diagnostics; the raw output is never placed in the chat page DOM, composer, AI messages, ledgers, logs, or persistent extension storage. Skill helpers and AI responses cannot invoke this operation, and installation never uses tmux, Force run, or shell duplicate ledgers.
