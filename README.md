...
- The local server clamps the shell state timeout to 1 second through 10 minutes. This is not a command runtime limit: after that window, the server keeps waiting while the tmux runner process is still alive. Shell runners are serialized per resolved tmux pane, so after a page refresh a new helper waits safely behind the old page's still-running command instead of being typed into the busy terminal; queue waiting does not consume the new runner's state timeout, and the result reports `queued` / `queuedMs`. Different panes remain concurrent. Because a pane has only one foreground job, an indefinitely running command must exit or be interrupted with Ctrl+C before its queued successor starts. Ctrl+C is handled as a foreground runner interruption and returns immediately with `exitCode: 130`, `interrupted: true`, and `timedOut: false`. A server-controlled status acknowledgement also completes the request promptly if terminal capture loses the visible completion marker. `timedOut: true` is reserved for cases where neither completion nor a live runner can be confirmed after dispatch.
