...
- The local server clamps the shell state timeout to 1 second through 10 minutes. This is not a command runtime limit: after that window, the server keeps waiting while the tmux runner process is still alive. Ctrl+C is handled as a foreground runner interruption and returns immediately with `exitCode: 130`, `interrupted: true`, and `timedOut: false`. A server-controlled status acknowledgement also completes the request promptly if terminal capture loses the visible completion marker. `timedOut: true` is reserved for cases where neither completion nor a live runner can be confirmed.
