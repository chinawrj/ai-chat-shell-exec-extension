...
The source release includes a runtime-root-neutral `skill-creator` at `skills/skill-creator/SKILL.md`. Copy that directory into a persistent, writable Skill root of your choice, then set `AI_HELPER_SKILL_PATH` to that root (or include it in `AI_HELPER_SKILL_PATHS`) before starting the shell server. Avoid using a checkout or extracted release directory as the long-lived writable root: upgrades should not delete user-created Skills or dirty the repository. When the Skill is loaded, the shell server injects the authoritative resolved roots and configuration source into its instructions; the AI does not guess from tmux or browser environment. The Skill rejects empty configuration and asks for a target when multiple roots are valid and no existing Skill identifies one uniquely.
