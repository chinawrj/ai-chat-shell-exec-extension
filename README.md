...
- The local server clamps the shell state timeout to 1 second through 10 minutes. This is not a command runtime limit: after that window, the server keeps waiting while the tmux runner process is still alive. Shell runners are serialized per resolved tmux pane with ownership stored in tmux, so after a page refresh or shell-server restart a new helper waits safely behind the old page's still-running command instead of being typed into the busy terminal. Manual-busy detection is authoritative only when tmux supplies the pane PID/TTY and the server can prove a distinct foreground process group; missing readiness metadata fails closed and remains retryable. A blocking builtin run by the root interactive shell itself (for example direct `read` or a pure-builtin loop) has the same PID/process group as the prompt and cannot be distinguished without shell-hook integration. Put such work in a script or child shell, or use a dedicated pane before sending helpers. Queue waiting does not consume the new runner's state timeout, and the result reports `queued` / `queuedMs`; direct Terminal vision test runs share the same queue. Different panes remain concurrent. An observed indefinitely running foreground child must exit or be interrupted with Ctrl+C before its queued successor starts. Ctrl+C is handled as a foreground runner interruption and returns immediately with `exitCode: 130`, `interrupted: true`, and `timedOut: false`; interruption before the executed marker remains retryable, while interruption after it is completed execution history. A server-controlled status acknowledgement also completes the request promptly if terminal capture loses the visible completion marker. `timedOut: true` is reserved for cases where neither completion nor a live runner can be confirmed after dispatch.
...
- The frontend execution lock ends when the backend response has been captured. Shell/board results that cannot yet enter or leave the composer are stored in a bounded per-tab pending-delivery queue and retried locally without another backend run. A delivery may write its result into the composer only once. If you delete or replace that text, the extension treats it as an explicit cancellation, drops the current queued output batch, sends no presentation receipt, and never puts the content back; a genuinely new helper can still recover an unpresented server result. Automatic Send is a cancellable UI follow-up that retains ownership only while the composer still contains its exact inserted text; disconnect, overwrite (including an agent delivery overwrite), or a page-lifecycle change aborts before click/form/keyboard side effects. Saved send controls must still belong to the current composer after SPA changes. A stale composer node or delayed send-button calibration therefore cannot send somebody else's text or block a newly rendered helper that the server has never executed. File and agent helper operations also never re-execute merely because model-facing output could not be sent.
