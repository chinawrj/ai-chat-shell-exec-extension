...
6. For releases that need screenshots, run the e2e test with `AI_SHELL_E2E_SCREENSHOT_DIR`, then include the generated PNGs in the release assets:
   ...
   Use absolute `https://github.com/.../releases/download/vX.Y.Z/...png` image URLs in release notes, because relative repository paths do not render from GitHub release pages. Keep the README `Latest Extension Panel Screenshots` section pointing at the latest committed screenshot files.
