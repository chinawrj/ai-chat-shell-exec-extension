...
EXPECTED_SERVER_PROTOCOL_VERSION=11
EXPECTED_HELPER_PROTOCOL_VERSION=4
EXPECTED_SKILL_PROTOCOL_VERSION=1
...
if command -v curl >/dev/null 2>&1; then
  ...
  if [[ -n "$health" ]]; then
    ...
    node - "$health" "$MANIFEST_PATH" "$EXPECTED_SERVER_PROTOCOL_VERSION" "$EXPECTED_HELPER_PROTOCOL_VERSION" "$EXPECTED_SKILL_PROTOCOL_VERSION" <<'NODE'
const fs = require("node:fs");
...
const expectedSkillProtocolVersion = Number(process.argv[6]);
...
const skillProtocolVersion = Number(health.skillProtocolVersion);
...
console.log(`info: Skill helper protocolVersion ${Number.isFinite(skillProtocolVersion) ? skillProtocolVersion : "(missing)"}`);
console.log(`info: Skill catalog v${health.skillCatalogVersion || 0}, count ${health.skillCount || 0}, SHA ${health.skillCatalogSha || "(missing)"}`);
...
if (skillProtocolVersion === expectedSkillProtocolVersion) {
  console.log("ok: Skill helper protocol version is supported");
} else {
  console.error(`error: Skill helper protocol version is unsupported or missing: ${Number.isFinite(skillProtocolVersion) ? skillProtocolVersion : "(missing)"}. Expected ${expectedSkillProtocolVersion}. Restart ./scripts/start_shell_server.sh from this checkout.`);
  process.exitCode = 1;
}
if (health.skillCatalogOk === true) {
  console.log("ok: Local Skill catalog is healthy");
} else {
  const firstSkillError = Array.isArray(health.skillCatalogErrors) ? health.skillCatalogErrors[0] : null;
  console.error(`error: Local Skill catalog is unhealthy: ${firstSkillError?.message || firstSkillError || "(missing error)"}`);
  process.exitCode = 1;
}
