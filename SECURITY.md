...
- The browser never adjudicates command duplicates from command text or prior output. It only prevents repeated DOM scans from resubmitting the exact same rendered helper request, including tracking DOM-node recycling and structurally rendered `shell-output`. The shell server makes the final decision after resolving the actual tmux pane, and only a command with a server-controlled completion proof on that pane instance and actual cwd can be considered duplicate. A command interrupted by Ctrl+C after its server-controlled executed marker is written is completed execution history and may be deduplicated; interruption before that marker remains runnable. Running, timed-out/unconfirmed, failed, unavailable-target, and incomplete-pane-identity attempts also remain runnable. Generic board prompts are spoofable, so board execution dedup is disabled and fails open; Force run explicitly bypasses a completed verdict where authoritative dedup is available.
- Shell helper runners are serialized by stable tmux socket + server-pid + pane-id identity. This prevents a refreshed page's new request from injecting a runner command into terminal input owned by an older foreground process; mutable window addresses cannot bypass the queue, and the target instance is revalidated after waiting so tmux reset/pane-id reuse fails safely instead of executing against a replacement pane. Different panes remain independent.
