...
- Composer ownership is exact-text and lifecycle scoped. A helper remains detected but unconsumed while a pending agent delivery owns the composer; it resumes after that delivery is submitted/acked or the page-agent profile changes. Already-submitted agent prompts are acknowledged from the rendered user message rather than reinserted.
